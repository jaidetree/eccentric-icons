...
(defn resize-copies
  ...
  [src-path dest-dir]
  (p/catch
    (p/let [cwd js/process.env.INIT_CWD
            src-path (.resolve path cwd src-path)
            src-img (.read jimp src-path)
            dest-dir (str (.resolve path cwd dest-dir) ".iconset")]
      (println {:src src-path :dest dest-dir})
      (p/do
